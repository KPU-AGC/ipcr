...
func ParseArgs(fs *flag.FlagSet, argv []string) (Options, error) {
	...
	// Performance
	fs.IntVar(&opt.Threads, "threads", runtime.NumCPU(), "number of worker threads (0 = all CPUs) [0]")
	...
	fs.BoolVar(&opt.Pretty, "pretty", false, "pretty ASCII alignment (text) [false]")
